...
def run_query(query,parameters=()):
    conn= psycopg2.connect(dbname="studentdb",user="krishankumar",password="",host="localhost", port="5432")
    cur= conn.cursor()
    query_result= None
    try:
...
def refresh_treeview():
    for item in tree.get_children(): #clear the current items in the treeview
        tree.delete(item)
...
frame.grid(row=0,column=0,padx=10,sticky="ew")
...
# tree view for dispay the database record at GUI
